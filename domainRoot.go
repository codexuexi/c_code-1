...
type WhoisOtherData struct {
	CreationTime        time.Time
	ExpirationTime      time.Time
	RegistrarServerName string
	DomainServer        []string
	Dns                 []string
	Email               string
	Phone               string
...
func WhoisExtData(whois_data string) (w WhoisOtherData) {
	s := whois_data
	s = strings.ReplaceAll(s, "\r", "")
	//得到注册时间
	create_time := regexp.MustCompile(`(?i)(crea|regi.*?time).*?:(.*?)\n`).FindAllStringSubmatch(s, 1)
	if len(create_time) > 0 {
		s2 := strings.Trim(create_time[0][2], " ")
		if _tmp_f, e := dateparse.ParseAny(s2); e == nil {
			w.CreationTime = _tmp_f
		}
	}
	//得到到期时间
	exp_time := regexp.MustCompile(`(?i)expi.*?(:|date::)(.*?)\n`).FindAllStringSubmatch(s, 1)
	if len(exp_time) > 0 {
		s2 := strings.Trim(exp_time[0][2], " ")
		if _tmp_f, e := dateparse.ParseAny(s2); e == nil {
			w.ExpirationTime = _tmp_f
		}
	}
	//获取注册的服务商名称
	registrservername := regexp.MustCompile(`(?i)Registrar:(.*?)\n`).FindStringSubmatch(s)
	if len(registrservername) == 2 {
		w.RegistrarServerName = registrservername[1]
	}
	//获取域名服务器
	domainserver := regexp.MustCompile(`(?i)registrar whois server:(.*?)\n`).FindAllStringSubmatch(s, -1)
	for _, do := range domainserver {
		if len(do) == 2 {
			w.DomainServer = append(w.DomainServer, do[1])
		}
	}
	//获取dns服务器
	domainserver = regexp.MustCompile(`(?i)name server:(.*?)\n`).FindAllStringSubmatch(s, -1)
	for _, do := range domainserver {
		if len(do) == 2 {
			w.Dns = append(w.Dns, do[1])
		}
	}
	//提取邮箱
	email := regexp.MustCompile(`(?i)Email:(.*?)\n`).FindStringSubmatch(s)
	if len(email) == 2 {
		w.Email = email[1]
	}
	//提取手机号
	phone := regexp.MustCompile(`(?i)Phone:(.*?)\n`).FindStringSubmatch(s)
	if len(phone) == 2 {
		w.Phone = phone[1]
	}
	//time_now := time.Now()
	//sub := time_now.Sub(w.ExpirationTime)
	//duration := w.ExpirationTime.Sub(time.Now())
	//duration.Seconds()
	//fmt.Println(sub, duration.String())
	return
}

//在whois 信息中提取时间
func WhoisRegisterDate(whois_data string) (creation_date time.Time, expiration_date time.Time) {
	s := whois_data
	s = strings.ReplaceAll(s, "\r", "")
	//得到注册时间
	create_time := regexp.MustCompile(`(?i)(crea|regi.*?time).*?:(.*?)\n`).FindAllStringSubmatch(s, 1)
